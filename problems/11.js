...
for (i = 0; i < 20; i++) {
  numsArray[i] = String(numsArray[i]).split(",");
}
// for문 돌리고
for (i = 0; i < 20; i++) {
  for (j = 0; j < 20; j++) {
    ...
    // result보다 많으면 더하기
    if (j < 17) {
      let plusHorizontalNum = 1;
      for (k = 0; k < 4; k++) {
        plusHorizontalNum *= Number(numsArray[i][j + k]);
      }
      if (plusHorizontalNum > result) {
        result = plusHorizontalNum;
      }
    }
    ...
    // result보다 많으면 더하기
    if (i < 17) {
      let plusPerpendicularNum = 1;
      for (k = 0; k < 4; k++) {
        plusPerpendicularNum *= Number(numsArray[i + k][j]);
      }
      if (plusPerpendicularNum > result) {
        result = plusPerpendicularNum;
      }
    }
    ...
    if (i < 17 && j < 17) {
      let diagonalNum = 1;
      for (k = 0; k < 4; k++) {
        diagonalNum *= Number(numsArray[i + k][j + k]);
      }
      if (diagonalNum > result) {
        result = diagonalNum;
      }
    }
